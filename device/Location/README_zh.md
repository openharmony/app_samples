...
4.因获取经纬度需要在室外且接口获取时间较长，所以测试需有GPS的测试机，服务器有需要guest网络，测试时需分两部分测试，连接guest网络，打开应用，抓取日志，http请求获取到结果则http测试成功，在室外点击国内热门城市上的定位图标，等几分钟，抓取日志，若经纬度打印出来不为0，则获取经纬度测试成功。